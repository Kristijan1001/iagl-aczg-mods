...
import requests, time, json, os, subprocess, re
...
class download(object):
	...
	class archive_org(object):
		...
		def download(self):
			...
			for cr in self.rom: #Threaded download for each rom in order
				if cr.get('continue_with_download'):
					...
					if self.get_aria2_path() is not None:
						self.download_with_aria2(cr)
						continue
		...
		def get_aria2_path(self):
			#Return the path to the bundled aria2c.exe, or None if not present
			try:
				p = Path(xbmcvfs.translatePath(self.config.addon.get('addon_handle').getAddonInfo('path'))).joinpath('resources','bin','aria2c.exe')
				if p.exists():
					return str(p)
			except Exception as exc:
				xbmc.log(msg='IAGL:  aria2 path error: {}'.format(exc),level=xbmc.LOGERROR)
			return None

		def download_with_aria2(self,cr):
			#Robust download via aria2c: resumable, retrying, size-verified. Sets cr['download_success'].
			aria2 = self.get_aria2_path()
			dest = Path(xbmcvfs.translatePath(str(cr.get('dl_filepath'))))
			dl_dir = str(dest.parent)
			out_name = dest.name
			total = cr.get('filesize') if isinstance(cr.get('filesize'),int) else None
			try:
				os.makedirs(dl_dir,exist_ok=True)
			except Exception:
				pass
			#Carry archive.org login cookies for restricted items
			cookie_header = None
			try:
				cks = ['{}={}'.format(c.name,c.value) for c in self.session.cookies]
				if cks:
					cookie_header = '; '.join(cks)
			except Exception:
				cookie_header = None
			conns = self.threads if isinstance(self.threads,int) and self.threads>0 else 5
			if conns>16:
				conns = 16
			cmd = [aria2,cr.get('url'),
				'--dir={}'.format(dl_dir),
				'--out={}'.format(out_name),
				'--max-connection-per-server={}'.format(conns),
				'--split={}'.format(conns),
				'--min-split-size=2M',
				'--max-tries=5',
				'--retry-wait=3',
			...
			if cookie_header:
				cmd += ['--header=Cookie: {}'.format(cookie_header)]
			xbmc.log(msg='IAGL:  aria2 downloading {} with {} connections'.format(out_name,conns),level=xbmc.LOGDEBUG)
			startupinfo = None
			creationflags = 0
			try:
				...
				startupinfo = None
			try:
				proc = subprocess.Popen(cmd,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,startupinfo=startupinfo,creationflags=creationflags,universal_newlines=True,bufsize=1)
			except Exception as exc:
				xbmc.log(msg='IAGL:  aria2 failed to start: {}'.format(exc),level=xbmc.LOGERROR)
				cr['download_success'] = False
				cr['download_size'] = None
				cr['download_message'] = 'aria2 failed to start'
				return False
			prog = {'pct':0,'speed':''}
			def _reader(pipe,prog):
				try:
					for line in iter(pipe.readline,''):
						m = re.search(r'\((\d+)%\)',line)
						if m:
							prog['pct'] = int(m.group(1))
						sm = re.search(r'DL:\s*([0-9.]+[A-Za-z]*)',line)
						if sm:
							prog['speed'] = sm.group(1)
				except Exception:
					pass
			rt = Thread(target=_reader,args=(proc.stdout,prog))
			rt.daemon = True
			rt.start()
			cancelled = False
			while proc.poll() is None:
				if self.dp is not None and self.dp.iscanceled():
					...
					break
				if self.dp is not None:
					pct = prog['pct'] if prog['pct']<100 else 99
					if total and total>0:
						self.dp.update(pct,'{}[CR]{} / {}[CR]{}/s'.format(out_name,self.bytes_to_string_size(int(total*pct/100.0)),self.bytes_to_string_size(total),prog.get('speed') or '0'))
					else:
						self.dp.update(pct,'{}[CR]{}[CR]{}/s'.format(out_name,self.bytes_to_string_size(dest.stat().st_size if dest.exists() else 0),prog.get('speed') or '0'))
				xbmc.sleep(400)
			try:
			...
			if cancelled:
				...
				self.delete_file(str(dest))
				self.delete_file(ctrl)
				cr['download_success'] = False
				cr['download_size'] = None
				cr['download_message'] = 'Download cancelled'
				return False
			final_size = dest.stat().st_size if dest.exists() else 0
			ok = (rc==0) and dest.exists() and final_size>0
			if ok and total and total>0 and final_size!=total:
				xbmc.log(msg='IAGL:  aria2 size mismatch got {} expected {}'.format(final_size,total),level=xbmc.LOGERROR)
				ok = False
			if ok:
				self.delete_file(ctrl)
				if self.dp is not None:
					self.dp.update(100,out_name)
				cr['download_success'] = True
				cr['download_size'] = final_size
				xbmc.log(msg='IAGL:  aria2 download complete {} ({} bytes)'.format(out_name,final_size),level=xbmc.LOGDEBUG)
				return True
			else:
				xbmc.log(msg='IAGL:  aria2 download failed rc={} size={} expected={}'.format(rc,final_size,total),level=xbmc.LOGERROR)
				self.delete_file(str(dest))
				self.delete_file(ctrl)
				cr['download_success'] = False
				cr['download_size'] = None
				cr['download_message'] = 'aria2 download failed (code {})'.format(rc)
				return False
